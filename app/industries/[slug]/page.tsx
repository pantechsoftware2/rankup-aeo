...
export default function IndustryLandingPage({
  ...
  const primaryService = depth?.primaryServiceSlug
    ? getLandingPage('service', depth.primaryServiceSlug)
    : undefined;
  const url = absoluteUrl(`/industries/${page.slug}`);
        ...
        {depth ? (
          <>
            <section className="mt-16 grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
              <div>
                <h2 className="text-3xl font-bold font-space">What this industry page covers</h2>
            ...
            <section className="mt-16 grid gap-8 lg:grid-cols-2">
              ...
              <InfoList title="Industry-specific problems" items={depth.problemsSolved} />
            </section>

            <section className="mt-16 grid gap-8 lg:grid-cols-3">
              <InfoList title="Search behavior" items={depth.searchBehavior || []} />
              <InfoList title="Trust signals" items={typeof depth.trustSignals === 'string' ? [depth.trustSignals] : depth.trustSignals || []} />
              <InfoList title="AI visibility concerns" items={depth.aiVisibilityConcerns || []} />
            </section>
            ...
            <section className="mt-16 rounded-3xl border border-white/10 bg-[#0A0A0A] p-8">
              ...
              <div className="flex flex-wrap gap-4">
                {primaryService ? (
                  <Link href={`/services/${primaryService.slug}`} className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-bold text-black transition hover:scale-105">
                    Related service
                    <ArrowRight className="h-4 w-4" />
                  </Link>
                ) : null}
                <Link href="/audit-flow" className="inline-flex items-center gap-2 rounded-full border border-white/10 px-5 py-3 text-sm font-bold text-white transition hover:border-white/20">
                  Run the audit
                  <ArrowRight className="h-4 w-4" />
                </Link>
                <Link href="/services" className="inline-flex items-center gap-2 rounded-full border border-white/10 px-5 py-3 text-sm font-bold text-white transition hover:border-white/20">
                  See services
                  <ArrowRight className="h-4 w-4" />
                </Link>
              </div>
